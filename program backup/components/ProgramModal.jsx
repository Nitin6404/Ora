...
const ProgramModal = ({ isOpen, onClose, programData = {} }) => {
    return (
        <ModalWrapper isOpen={isOpen} onClose={onClose} Header={<>
            <h1 className="hidden lg:block text-xl font-semibold text-gray-800">Assigned Patients</h1>
            <h2 className="hidden lg:block text-xl font-semibold text-gray-800">Onboarding Questionnaire Responses</h2>
        </>}
        >
                ...
                <div className="bg-white rounded-xl p-6 space-y-2 font-medium text-gray-500">
                    <div className="flex flex-col items-center justify-between">
                        <div className="flex items-center justify-between w-full">
                            ...
                            <span
                                className={`px-1 py-1 rounded-full text-xs font-medium flex justify-center items-center  ${getStatusColor(data?.status, data?.flagged)
                                    }`}
                            >
                        ...
                        <div className="flex items-center justify-start w-full pt-3">
                            <span className='text-[#7367f0] text-lg md:text-2xl font-medium'>{data.name || "N/A"}</span>
                        </div>
                    </div>
                    <Divider />
                    <div className='flex flex-col items-start justify-start text-black'>
                        <span className='text-sm'>Condition Type</span>
                        <span className='font-medium text-lg'>{data.condition_type || "N/A"}</span>
                    </div>
                    <Divider />
                    <div className='p-2 bg-[#f1f1fd] rounded-xl'>
                        <AdvisorInfo advisor={data.advisor || []} />
                    </div>
                    <InfoRow label="Target Group" value={data.target_group || "N/A"} className="bg-[#dff0fe] text-blue-600" />
                    <InfoRow label="Estimate Duration" value={data.estimate_duration || "N/A"} className="text-black font-bold" />
                    <Divider />
                    <ProgramDetailCard title="Therapy Goal" description={data.therapy_goal || "N/A"} />
                    <Divider />
                    <ProgramDetailCard title="Program Description" description={data.program_description || "N/A"} />
                </div>
            ) : (
...
const AdvisorInfo = ({ advisor }) => {
    return (
        <div className="font-medium text-black w-full flex items-center">
            <InfoRow label={advisor.advisor_type} value={advisor.name || "N/A"} className="bg-white" />
        </div>
    )
}
        ...
        <div className="flex items-center justify-between w-full">
            <div className="flex items-center justify-start gap-2 w-full">
                ...
                <div className='flex flex-col items-start justify-start p-2'>
                    <span className="font-medium flex items-center text-xl">{patient.full_name || "N/A"}</span>
                    <span className="text-gray-700 text-sm" >ORA-00{patient.patient_id || "N/A"}</span>
                </div>
            </div>
            <div className='w-6 h-6'>
                <img src="/tilde-icon.png" alt="" className="w-full h-full object-cover" />
            </div>
        </div>
    );
};
        ...
        <div className="bg-white rounded-xl p-6 flex flex-col items-start justify-start text-black h-full w-full">
            ...
            {questions.length > 0 ? (
                <>
                    ...
                    <QuestionCard label="Overall Response Rate" analytics={true} questions={questions} />
                    <QuestionCard label="Top-Level Answer Trend (for each question)" analytics={false} questions={questions} />
                </>
            ) : (
                ...
                {analytics ? (
                    <>
                        <div className="flex items-center justify-between w-full py-2">
                            <span className="font-medium flex items-center text-sm">Metric</span>
                            <span className="font-medium flex items-center text-sm">Response</span>
                        </div>
                        <Divider />
                    ...
                    <>
                        <div className="flex items-center justify-between w-full py-2">
                            <span className="font-medium flex items-center text-sm">Question</span>
                            <div className="flex items-center gap-5">
                                <span className="font-medium flex items-center text-sm">Option A</span>
                                <span className="font-medium flex items-center text-sm">Option B</span>
                            </div>
                        </div>
                        <Divider />
...
const QuestionRow = ({ question, analytics }) => {
    return (
        <div className="flex items-center justify-between w-full py-1">
            <div className="flex items-center text-sm w-2/3">{question.question || "N/A"}</div>
            {analytics ? (
                <span className="text-sm">{question.answer || "N/A"}</span>
            ) : (
                <div className='flex justify-center items-center gap-5 w-1/3'>
                    {question.answers.map((answer, index) => (
                        <span className="text-gray-700 text-sm uppercase text-nowrap" key={index}>{answer.label || "N/A"}</span>
                    ))}
                </div>
            )}
        </div>
                ...
                <div className="flex items-center justify-start gap-4 w-full pb-3 ">
                    {options.map((option, index) => (
                        <button
                            ...
                            ${selectedOption === option ? "bg-gradient-to-b from-[#7367F0] to-[#453E90] text-white shadow-md" : "bg-white text-black hover:bg-gray-100 hover:text-black"}
                            `}
                            key={index}>
...
const InfoRow = ({ label, value, className }) => (
    <div className="flex justify-between w-full">
        ...
        <span className={`font-medium px-3 py-1 rounded-full bg-white uppercase ${className}`}>{value}</span>
    </div>
);
const Divider = ({ className }) => <div className={`h-[3px] w-full bg-gray-200 ${className}`} />;
