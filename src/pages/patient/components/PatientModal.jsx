...
const PatientDashboardModal = ({ isOpen, onClose, patientData }) => {
  ...
  return (
    <ModalWrapper isOpen={isOpen} onClose={onClose} Header={
      <>
        <h1 className="hidden lg:block text-xl font-semibold text-gray-800">Program Participation Summary</h1>
        <h2 className="hidden lg:block text-xl font-semibold text-gray-800">Session Log Table</h2>
      </>
    }>
...
const ProgramCard = ({ program, index }) => (
  <div key={index} className="bg-white border-2 border-[#7367f0] rounded-xl p-6 shadow-sm min-h-[300px]">
    <div className="flex items-center justify-between mb-2">
      <div className="flex items-center justify-between w-full">
        ...
        <span
          className={`px-1 py-1 rounded-full text-xs font-medium flex justify-center items-center  ${getStatusColor(program.status, program.flagged)
            }`}
        >
    ...
    <div className="space-y-2 mt-3 font-medium text-black">
      ...
      <MoodTrend label="Mood Trend" moodTrend={{
  ...
  return (
    <div className="bg-white rounded-xl p-6 space-y-4 h-full lg:overflow-y-auto no-scrollbar ">
      {data.sessions.length > 0 ? (
        <>
          <h3 className="font-medium text-gray-800 mb-2">A timeline of all sessions from all programs</h3>
...
const SessionCard = ({ session, index }) => (
  <>
    <div key={index} className="bg-white border-2 border-[#7367f0] rounded-xl p-[1.5em] shadow-sm min-h-[18.75em] z-[100]">
        ...
        <div className="flex justify-between items-center">
          <div className="flex flex-col">
            {session.breakTaken && (
              <div className="flex justify-between items-center gap-2">
                ...
                <span className="font-medium text-red-600 px-[1.25em] py-[0.25em] rounded-full bg-red-50">{session.breakTaken}</span>
              </div>
            )}
          ...
          <div className="flex flex-col">
            {session.distress_flag && (
              <div className="flex justify-between items-center gap-2">
                <span className="text-black font-medium">Distress Flag Raised</span>
                <span className="font-medium text-red-600 px-[1.25em] py-[0.25em] rounded-full bg-red-50">{session.distress_flag}</span>
              </div>
            )}
...
const InfoRow = ({ label, value, className='bg-white', textClassName='' }) => (
  <div className="flex justify-between">
    <span className="text-gray-600 font-medium">{label}</span>
    <span className={`font-medium px-[1em] py-[0.2em] rounded-full ${className} ${textClassName}`}>{value}</span>
  </div>
);
const PatientProgramInfo = ({ program, duration }) => {
  return (
    <div className="space-y-2 mt-3 font-medium text-black">
      <InfoRow label="Program" value={program?.program} className="bg-[#7367f0]/20" textClassName="text-[#7367f0]" />
      <InfoRow label="Environment" value={program?.environment} className="bg-cyan-50" textClassName="text-cyan-500" />
      {!duration ? (
        <InfoRow label="Sessions" value={program?.sessions} className="bg-white" textClassName="text-black" />
      ) : (
        <InfoRow label="Duration" value={program?.duration} className="bg-white" textClassName="text-black" />
      )}
