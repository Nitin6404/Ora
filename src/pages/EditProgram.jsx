...
useEffect(() => {
  const fetchProgram = async () => {
    ...
    try {
      ...
      // 🔐 Set ID counter to avoid duplicate IDs
      const maxSuffix = Math.max(
        ...nodesFromAPI.map((node) => {
          const match = node.id.match(/node_(\d+)/);
          return match ? parseInt(match[1], 10) : 0;
        }),
        0
      );
      id = maxSuffix + 1; // ✅ this resets the global id counter
    } catch (error) {
  ...
  const fullData = {
     name: "Cognitive Therapy-updated",     // <- You can make this dynamic
    is_active: true,   
    program_data: {
      nodes: nodes.map((n) => ({
        id: n.id,
...
const programId = pathParts[pathParts.length - 1];
  ...
  `https://a4do66e8y1.execute-api.us-east-1.amazonaws.com/dev/api/program/programs/${programId}/`,
  {
    ...
    console.log("✅ Program updated:", responseData);
