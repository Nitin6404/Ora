...
const ProgramCard = ({ program, onClick }) => {
    ...
    //         if (open && !event.target.closest('.program-card')) {
    //             setOpen(false);
    //         }
    //     };
    ...
    //     return () => {
    //         document.emoveEventListener('mousedown', handleClickOutside);
    //     };
    // }, [open]);
    return (
        <div className="rounded-2xl border border-gray-200 bg-white shadow-sm p-4 hover:shadow-md hover:bg-[#f1f1fd] transition-all duration-300 ease-in-out w-full">
            ...
            <div className="flex items-center justify-between mb-[0.5em]">
                <span className="text-xs font-medium text-gray-600">{"ORA-00" + programId}</span>
                <div className="flex items-center gap-[0.5em]">
                    <div className={`flex items-center gap-[0.25em]  text-xs  p-1 rounded-full font-medium
                        ${program.status === "draft" ? "bg-[#fca29a] text-[#7A271A]" : "bg-[#D5D7DA] text-gray-800"}
                        `}>
                        {program.status === "draft" ? <CirclePause size={24} className="p-1 bg-white rounded-full" /> : <Zap size={24} className="p-1 bg-white rounded-full" />}
                        <span className="pr-1">{status}</span>
                    </div>
                    <div className="relative">
                        <EllipsisVertical size={20} className="text-gray-400 cursor-pointer" onClick={() => setOpen(!open)} />

                        {/* Dropdown menu */}
                        {open && (
                            <div className="absolute top-3 right-0 z-10 mt-2 w-36 bg-white rounded-md shadow-lg p-1">
                                <button
                                    className="flex items-center gap-2 w-full p-2 text-sm text-gray-700 hover:bg-gray-100 cursor-pointer"
                                    onClick={(e) => {
                                        e.stopPropagation();
                                        console.log("Edit program ID:", program.id);
            ...
            <h3 className="text-lg font-medium text-[#414651] leading-[28px]">
                {program.name.length > 22 ? (program.name).slice(0, 22).concat("...") : program.name}
            </h3>
            ...
            <p className="flex justify-between items-center gap-[0.5em] text-xs font-medium leading-[18px] text-[#414651]">
                {/* Show date in format May 6, 2025 */}
                Last Updated: {new Date(program.updated_date).toLocaleDateString("en-US", {
                    year: "numeric",
                    month: "long",
                    day: "numeric",
                }) || "N/A"}
