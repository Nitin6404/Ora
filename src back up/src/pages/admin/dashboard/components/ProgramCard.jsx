    ...
    <div className="rounded-3xl border border-gray-200 bg-[#fff] shadow-sm p-4 transition-all hover:shadow-md overflow-hidden">
        ...
        <div className="flex items-center space-x-2">
          ...
          >
            <div className="mr-1 bg-white rounded-full w-6 h-6 flex items-center justify-center">
              {program.flagged && <TriangleAlert className="w-4 h-4" />}
      ...
      <div className="flex justify-between items-center mb-3">
        ...
        <div className="flex items-center bg-white rounded-full px-2 py-1">
          <span>{getMoodEmoji(program.mood)}</span>
          <span className="text-sm text-gray-600">{program.mood}</span>
        </div>
      </div>
      ...
      <p className="text-base text-gray-500 mb-3">{program.program}</p>
      ...
      <div className="space-y-1 text-xs">
        ...
        <div className="flex justify-between"> 
          <div className="flex flex-col justify-between text-gray-500">
            ...
            <span className="px-1 py-1.5 rounded-full bg-[#f2f0fd] text-center mt-2">{program.session}</span>
          </div>
          <div className="w-px h-12 bg-gray-200" />
          <div className="flex flex-col justify-between text-gray-500">
            <span>Exit & Ground Used</span>
            <span className="px-1 py-1.5 rounded-full bg-[#eff8ff] text-center mt-2">{program.exits}</span>
          </div>
        </div>
        <div className="border-b border-gray-200 mt-3"></div>
        <div className="pt-2 text-[11px] text-black">
          Last Session: {program.lastSession}
        </div>
      </div>
    </div>
  );
