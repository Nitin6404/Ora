...
import { CONDITION_OPTIONS, DURATION_OPTIONS, TARGET_GROUP_OPTIONS, BREADCRUMBS } from "../../../constants";
...
export default function EditProgram() {
    ...
    const handleSubmit = async (isSaveAsDraft = false) => {
        ...
        try {
            ...
            if (isSaveAsDraft) {
                ...
                const res = await axiosInstance.post(API_BASE_URL + ENDPOINTS.program, multipartData, {
                    headers: {
                ...
                navigate(`/programs/edit-decision-tree-flow/${id}`, { state: { programDetails: dataToSend } });
            }
        } catch (err) {
    ...
    const fetchProgramDetails = async () => {
        ...
        try {
            const res = await axiosInstance.get(`${API_BASE_URL}${ENDPOINTS.program}/${id}`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            })
            console.log("res.data.advisor_type", res.data.advisor_type)
            const formData = {
                ...res.data,
                ...(res.data.advisor.advisor_type === "VMA" ? { vma: res.data.advisor.id, vsa: "" } : { vsa: res.data.advisor.id, vma: "" })
            }
        ...
        const fetchAllData = async () => {
            try {
                const [metaData] = await Promise.all([
                    fetchData(ENDPOINTS.metaData),
                ]);

                const vma = metaData?.advisors.filter((item) => item.advisor_type === "VMA");
                const vsa = metaData?.advisors.filter((item) => item.advisor_type === "VSA");
                ...
                <div className="sticky top-0 z-[999] p-4 md:px-6 md:py-3 backdrop-blur-sm md:ml-4 h-full">
                    <div className=" flex flex-col gap-2 h-full">
                        ...
                        <div
                            ...
                            className="w-full backdrop-blur-sm bg-white/10 rounded-[15px] lg:rounded-[24px] px-2 py-2 md:px-4 lg:px-3 lg:pt-2 lg:pb-8 h-[85%]"
                        >
                                    ...
                                    <button
                                        ...
                                        onClick={() => {
                                            if (item.href === "/edit-decision-tree-flow") {
                                                ...
                                                delete dataToSend.vsa;
                                                console.log("dataToSend", dataToSend    )
                                                navigate(`${item.href}/${id}`, { state: { programDetails: dataToSend } });
                                            }
                                        }}
                                    >
                                        {item.name}
                            ...
                            <div className="bg-white/30 mx-2 md:mx-4 lg:px-8 lg:py-4 !rounded-[15px] lg:rounded-[16px] mt-6 h-[90%] flex flex-col justify-between">
                                {loading ? (
                                    <div className="flex justify-center items-center py-10 w-full h-full col-span-4">
                                        <PrimaryLoader />
                                    </div>
                                ) : (
                                            ...
                                            <div className="flex flex-col">
                                                {vmaList.length > 0 && (
                                                    <CustomDropdown
                                                        ...
                                                        options={vmaList}
                                                        selected={vmaList.find((item) => item.id === formData.vma)?.name}
                                            ...
                                            <div className="flex flex-col">
                                                {vsaList.length > 0 && (
                                                    <CustomDropdown
                                                        ...
                                                        options={vsaList}
                                                        selected={vsaList.find((item) => item.id === formData.vsa)?.name}
                                ...
                                <div className="flex flex-col gap-4 sm:flex-row sm:justify-between py-6 border-t border-[#ABA4F6]">
                                    <button
                                        onClick={() => navigate(-1)}
