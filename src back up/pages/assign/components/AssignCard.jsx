...
import { Zap, EllipsisVertical, CirclePause, AlertTriangle, CircleCheckBig, Clock3 } from "lucide-react";
        ...
        <div className="rounded-2xl p-[1rem] shadow-md border bg-white w-full hover:bg-[#f1f1fd] transition-all duration-300 ease-in-out">
            ...
            <div className="flex items-center gap-3 mb-[0.5rem]">
                <div className="w-12 h-12 rounded-full bg-gray-200 flex items-center justify-center text-xs font-semibold text-gray-600">
                    {assign.patient?.profile_image ? <img src={assign.patient.profile_image} alt="Profile" className="w-full h-full rounded-full" /> : "U"}
                </div>
                <div className="ml-auto text-sm bg-[#f8f7fd] px-3 py-1.5 rounded-full">{assign.mood_status || "😐 Sad"}</div> {/* mood_status is null; fallback */}
            </div>
            ...
            <div className="flex flex-col items-start gap-1 mb-2">
                ...
                <div className="text-sm text-[#3e404c]">{assign.program?.name || "The Mindful Coping Starter"}</div>
            </div>
            ...
            <div className="bg-[#f5f5f5] w-full flex flex-col items-start justify-between px-2 py-4 rounded-2xl">
                ...
                <div className="text-xs space-y-2 w-full">
                    <div className="flex gap-2 items-center justify-between">
                        <span className="bg-gray-100 font-semibold px-2 py-1 rounded-full">VMA</span>
                        <span className="text-right bg-white font-medium px-5 py-1 rounded-full">{assign.vma_detail?.name || "N/A"}</span>
                    </div>
                    <div className="flex gap-2 items-center justify-between">
                        <span className="bg-gray-100 font-semibold px-2 py-1 rounded-full">VSA</span>
                        <span className="text-right bg-white font-medium px-5 py-1 rounded-full">{assign.vsa_detail?.name || "N/A"}</span>
                    </div>
                    ...
                    <div className="flex gap-2 items-center justify-between w-full">
                        ...
                        <span className="text-sm font-medium text-[#2d4e57] bg-[#ebf0f4] px-5 py-1 rounded-full">{assign.solfeggio_frequency || "N/A"}</span>
                    </div>
                </div>
            </div>
            ...
            <div className="space-y-1 text-xs">
                ...
                <div className="flex justify-between text-[#42424e]">
                    <div className="flex flex-col justify-between font-medium">
                        ...
                        <span className="px-4 py-1.5 rounded-full bg-[#f2f0fd] text-center mt-2 text-[#7670b8]">{assign.number_of_sessions + " of 3" || "N/A"}</span>
                    </div>
                    <div className="w-[1.5px] h-12 bg-gray-200" />
                    <div className="flex flex-col justify-between font-medium">
                        <span>Break Taken</span>
                        <span className="px-4 py-1.5 rounded-full bg-[#eff8ff] text-center mt-2 text-[#5d84a3]">{assign.exits || "2 Breaks"}</span>
                    </div>
                </div>
                ...
                <div className="py-2 text-xs font-semibold text-[#3a3c48] flex justify-between items-center">
                    <span>Last Session: {assign.last_session || "May 20, 2025"}</span>
